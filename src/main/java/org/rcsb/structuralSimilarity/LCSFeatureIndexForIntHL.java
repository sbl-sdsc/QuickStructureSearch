 ...
 */
public class LCSFeatureIndexForIntHL implements PairFunction<Tuple2<Integer,Integer>,String,Float> {
     ...
     */
	public LCSFeatureIndexForIntHL(Broadcast<List<Tuple2<String,Vector>>> data,int traceback) {
		this.data = data;
     ...
     */
	public LCSFeatureIndexForIntHL(Broadcast<List<Tuple2<String,Vector>>> data, double diff) {
		this.data = data;
		this.diff = diff;
	}
	 ...
	 */
	private int LCS(int[] v1, int[] v2, double diff) {
	...
	private void printLCS(int[] v1, int[] v2, int[][] b) {
		int x = v1.length - 1;
		int y = v2.length - 1;
