...
public class TestSetCreator1 { 
	...
	public static void main(String[] args ) throws FileNotFoundException
	{
		...
		TestSetCreator1 creator = new TestSetCreator1();
	...
	private void run(String path, String outputFileName, int nPairs, int seed) throws FileNotFoundException {
		...
		Class<?>[] classes = {SimplePolymerChain.class, IntegerToByteTransform.class, IntegerDeltaZigzagVariableByte.class};
        ...
        List<Tuple2<String, SimplePolymerChain>> chains = sc
        		...
        		.filter(t -> t._2.getCoordinateLength() >= 50 && t._2.getCoordinateLength() <= 500)
				.collect(); // return results to master node
        
		// Step 2.  broadcast feature vectors to all nodes
		final Broadcast<List<Tuple2<String, SimplePolymerChain>>> chainsBc = sc.broadcast(chains);
		...
		for (int i = 0; i < nPairs; i+=BATCH_SIZE) {
			...
			List<Tuple2<String, Float[]>> list = sc
					...
					.filter(new SimplePolymerChainPairLengthFilter(chainsBc, 0.5, 1.0)) // restrict the difference in chain length
					.mapToPair(new SimplePolymerChainPairToTmMapper(chainsBc)) // maps pairs of chain id indices to chain id, TM score pairs
					//				.filter(s -> s._2 > 0.9f) //
