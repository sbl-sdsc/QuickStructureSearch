 ...
 * @author  Peter Rose
 */
public class ChainPairToRmsdMapper implements PairFunction<Tuple2<Integer,Integer>,Integer,Float> {
	...
	public ChainPairToRmsdMapper(Broadcast<List<Tuple2<String,Point3d[]>>> data, GenericFingerprint fingerprint, int fragmentLength) {
		this.data = data;
	 ...
	 */
	public Tuple2<Integer, Float> call(Tuple2<Integer, Integer> tuple) throws Exception {
 ...
 //       float rmsd2 = (float) SuperPosition.calcRmsd(fragment1, fragment2);
        
		superposer.set(fragment1, fragment2);
        float rmsd = (float) superposer.getRmsd();
	
//        rmsd = Math.abs(rmsd-rmsd2);
        return new Tuple2<Integer,Float>(match, rmsd);
    }
