 ...
 */
public class LogSmithWatermanMapper implements AlignmentAlgorithmInterface {
	private static final long serialVersionUID = 1L;
	private Broadcast<Map<String,SequenceFeatureInterface<?>>> sequences = null;
	private double open = 1.1;
	private double extend = .1;
	public static void main(String[] args) {
		
	}
	public LogSmithWatermanMapper() {
	}

	public LogSmithWatermanMapper(Broadcast<Map<String,SequenceFeatureInterface<?>>> sequences) {
		this.sequences = sequences;
	}

	public String getName() {
		return "LogSmithWaterman";
	}
	...
	public Tuple2<String, Float> call(Tuple2<String, String> tuple) throws Exception {
		...
		float value = logSmithWatermanIndex(v1, v2);
...
//
	 ...
	 * @return
	 */
	private <T> float logSmithWatermanIndex(SequenceFeatureInterface<T> s1, SequenceFeatureInterface<T> s2) {
...
//		return (float) HashingJaccardIndex.HashingJaccardIndex(features1, features2);
		...
		return  (float)LogSmithWaterman.distance(seq1,seq2,open,extend);
